...
@Slf4j
public class ReferentialNotificationElasticsearchProcessor extends AbstractElasticsearchProcessor<String, Referential> {

    private final ISO8601DateFormat df = new ISO8601DateFormat();
    private final RetentionLevel retentionLevel;

    public ReferentialNotificationElasticsearchProcessor(ESBuffer esBuffer, ESErrorRetryWriter esErrorRetryWriter, RetentionLevel retentionLevel) {
    ...
    public void process(String key, Referential referential) {
        try {
            ...
            processToElasticsearch(df.parse(timestamp), referential.getProject(), referential.getType(), retentionLevel, valueAsString);
