...
export function MetricProvider({ children }) {
    const metricProvider = useMetricProvider();
    return <MetricContext.Provider value={metricProvider}>{children}</MetricContext.Provider>
}
export const useMetric = () => {
    return useContext(MetricContext)
}

const useMetricProvider = () => {
    ...
    const findConusmerState = async idProcess => {
        try {
            const res = await fetch(`${url.API_URL}/metric/findConsumerState?idProcess=${idProcess}`, {
                method: "GET",
    ...
    const scaleUp = async idProcess => {
        try {
            const res = await fetch(`${url.API_URL}/metric/scaleup?idProcess=${idProcess}`, {
                method: "GET",
                headers: requestHeaders,
            })
            const data = await res.json();
            ...
            console.log("Error while fetching '/metric/scaleup'");
        }
    }

    const scaleDown = async idProcess => {
        try {
            const res = await fetch(`${url.API_URL}/metric/scaledown?idProcess=${idProcess}`, {
                method: "GET",
                headers: requestHeaders,
            })
            const data = await res.json();
            if (data.status === 404) {
                console.log("Error while fetching '/metric/scaledown'");
                return
            }
            dispatch({ type: "SET_CONSUMER_STATE", payload: data });
        } catch (error) {
